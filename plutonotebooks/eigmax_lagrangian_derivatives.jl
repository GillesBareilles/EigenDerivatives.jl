...
function ∇²ϕᵢⱼ(M::EigmaxLinearManifold, x, d, i, j)
    ...
    for l in axes(res, 1), k in (M.r + 1):(M.pb.m)
        ...
        res[l] +=
            scalar * (τ(♈(i), ♈(k)) * ν(♈(j), ♈(k), l) + ν(♈(i), ♈(k), l) * τ(♈(j), ♈(k)))
    end
...
begin
    ...
    function ∇²Lagrangian_improved(
        pb::EigmaxLinear, M::EigmaxLinearManifold, x::Vector{Tf}, λ, d
    ) where {Tf}
        M.xref .= x
...
function plot_ϕ(x)
    ...
    model_to_functions = OrderedDict{String,Function}(
        ...
        "phi i,j - 2" =>
            t -> norm(
                ϕᵢⱼ(M, φ(t), i, j) - ϕᵢⱼ(M, φ(0), i, j) - t * Dϕᵢⱼ(M, φ(0), d, i, j) -
                0.5 * t^2 * DDϕᵢⱼ(M, φ(0), d, i, j),
            ),
        "phi k,l - 2" =>
            t -> norm(
                ϕᵢⱼ(M, φ(t), k, l) - ϕᵢⱼ(M, φ(0), k, l) - t * Dϕᵢⱼ(M, φ(0), d, k, l) -
                0.5 * t^2 * DDϕᵢⱼ(M, φ(0), d, k, l),
            ),
    )
...
function plot_ϕ2(x)
    ...
    model_to_functions = OrderedDict{String,Function}(
        ...
        "phi i,j - 2" =>
            t -> norm(
                ϕᵢⱼ(M, φ(t), i, j) - ϕᵢⱼ(M, φ(0), i, j) - t * cgradᵢⱼ - 0.5 * t^2 * chessᵢⱼ,
            ),
        "phi k,l - 2" =>
            t -> norm(
                ϕᵢⱼ(M, φ(t), k, l) - ϕᵢⱼ(M, φ(0), k, l) - t * cgradₖₗ - 0.5 * t^2 * chessₖₗ,
            ),
    )
...
function plot_h(x)
    ...
    model_to_functions = OrderedDict{String,Function}(
        ...
        "h ij" =>
            t -> norm(
                hᵢⱼ(M, φ(t), i, j) - hᵢⱼ(M, φ(0), i, j) - t * cgradᵢⱼ - 0.5 * t^2 * chessᵢⱼ,
            ),
        "h kl" =>
            t -> norm(
                hᵢⱼ(M, φ(t), k, l) - hᵢⱼ(M, φ(0), k, l) - t * cgradₖₗ - 0.5 * t^2 * chessₖₗ,
            ),
    )
...
function plot_lagrangian(x, λ)
    ...
    model_to_functions = OrderedDict{String,Function}(
        ...
        "lagrangian" =>
            t -> norm(
                lagrangian(pb, M, φ(t), λ) - lagrangian(pb, M, φ(0), λ) - t * cgrad -
                0.5 * t^2 * chess,
            ),
        "lag fast" =>
            t -> norm(
