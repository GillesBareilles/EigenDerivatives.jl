...
function plot(x)
	...
	model_to_functions = OrderedDict{String, Function}(
		...
		"t2" => t -> t^2,
		"t3" => t -> t^3,
		"phi i,j - 2" => t -> norm(NSP.ϕᵢⱼ(M, φ(t), i, j) - NSP.ϕᵢⱼ(M, φ(0), i, j) - t * NSP.Dϕᵢⱼ(M, φ(0), i, j, d) - 0.5 * t^2 * dot(NSP.∇²ϕᵢⱼ(M, φ(0), i, j, d), d)),
		"phi k,l - 2" => t -> norm(NSP.ϕᵢⱼ(M, φ(t), k, l) - NSP.ϕᵢⱼ(M, φ(0), k, l) - t * NSP.Dϕᵢⱼ(M, φ(0), k, l, d) - 0.5 * t^2 * dot(NSP.∇²ϕᵢⱼ(M, φ(0), k, l, d), d)),
